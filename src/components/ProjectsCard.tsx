...
export default function ProjectsCard({ image, link, imageWidth, imageHeight, header, subheader, description }: BoxProps) {
    ...
    <Grid item xs="auto">
      ...
      <GridItem raised>
        <BoxHeader className="box-headers">
          <div>
            <Header className='projects-box-header' >{header}</Header>
          </div>
        </BoxHeader>
      ...
      >
        <Fade in={open}>
          <Box sx={modalStyle}>
          <ModalHeader className='box-header' >{header}</ModalHeader>
            ...
            {link ? (
            <Link><a href={link} target="_blank" rel="noopener noreferrer">Link to project here!</a></Link>
            ) : null}
